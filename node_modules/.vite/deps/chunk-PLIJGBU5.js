...
function handle_error(error) {
  ...
  if (false_default && error instanceof Error && !adjustments.has(error)) {
    adjustments.set(error, get_adjustments(error, effect2));
  }
  if ((effect2.f & REACTION_RAN) === 0 && (effect2.f & EFFECT) === 0) {
    if (false_default && !effect2.parent && error instanceof Error) {
...
function invoke_error_boundary(error, effect2) {
  ...
  if (false_default && error instanceof Error) {
    apply_adjustments(error);
  }
  throw error;
}
  ...
  if (typeof value === "object" && value !== null) {
    ...
    if (is_array(value)) {
      ...
      for (var i = 0; i < value.length; i += 1) {
        var element = value[i];
        if (i in value) {
          copy[i] = clone(element, cloned, false_default ? `${path}[${i}]` : path, paths, null, no_tojson);
        }
      }
      return copy;
    }
    ...
    value.toJSON === "function" && !no_tojson) {
      return clone(
        ...
        value.toJSON(),
        cloned,
        false_default ? `${path}.toJSON()` : path,
        paths,
...
function async_derived(fn, label2, location) {
  ...
  if (false_default) signal.label = label2 ?? fn.toString();
  ...
  async_effect(() => {
    ...
    const handler = (value, error = void 0) => {
      ...
      if (error) {
        ...
        if (false_default && location !== void 0) {
  ...
  #commit() {
    ...
    for (let batch = first_batch; batch !== null; batch = batch.#next) {
      ...
      if (others.length === 0) {
        ...
        if (false_default && !batch.#decrement_queued) {
...
function update_effect(effect2) {
  ...
  try {
    ...
    if (false_default && tracing_mode_flag && (effect2.f & DIRTY) !== 0 && effect2.deps !== null) {
...
function proxy(value) {
  ...
  var version = state(0);
  var stack2 = false_default && tracing_mode_flag ? get_error("created at") : null;
  ...
  return new Proxy(
    ...
    value,
    {
      defineProperty(_, prop, descriptor) {
        ...
        if (s === void 0) {
          with_parent(() => {
            ...
            sources.set(prop, s2);
            if (false_default && typeof prop === "string") {
      ...
      get(target, prop, receiver) {
        ...
        if (false_default && prop === PROXY_PATH_SYMBOL) {
          return update_path;
        }
        var s = sources.get(prop);
...
//# sourceMappingURL=chunk-PLIJGBU5.js.map
